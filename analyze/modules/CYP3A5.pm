...
sub callCYP3A5($$$){
		...
		@ordered = ("CYP3A5s2,38","CYP3A5s3,85","CYP3A5s3Bs8,105","CYP3A5s3Bs8,114","CYP3A5s6,109","CYP3A5s7,10","CYP3A5s9,103");	
		...
		if($message eq ""){
			return "CYP3A5|$message|$finalCall|||||||\n";
		}
		else{
			return "CYP3A5|$message||||||||\n";
		}
...
sub Format($$){
my($call1, $call2) = @_;
	my (@unordered, @ordered, @numericOrder, $call, $digit);
	@numericOrder = ("1","2","3","3B","6","7","8","9");
	for (my $i=0; $i<scalar @numericOrder; $i++){		
		if($numericOrder[$i] eq $call1 |$numericOrder[$i] eq $call2){		
			push(@unordered, $i);			
		}
	}
	@ordered = sort {$a <=> $b} @unordered;	
	if(! defined $ordered[1]){
		$call = "*$numericOrder[$ordered[0]]|*$numericOrder[$ordered[0]]";
	}
	else{
		$call = "*$numericOrder[$ordered[0]]|*$numericOrder[$ordered[1]]"; # corrects to allow formatting of *3B	
	}
return $call;
}# end checkFormatting 
